...
namespace YU.Core.Utils
{
    public static class DBUtils
    {
        ...
        /// <exception cref="Exception"></exception>
        public static T ExecuteScalar<T>(string sql, T defaultValue = default(T), params SQLiteParameter[] parameters)
        {
            using (SQLiteConnection conn = new SQLiteConnection(connectionString))
            {
                using (SQLiteCommand cmd = new SQLiteCommand(conn))
                {
                    ...
                    return cmd.ExecuteScalar().TryPareValue<T>(defaultValue);
                }
            }
        }

        /// <summary> 
        /// 执行一个查询语句，返回一个包含查询结果的DataTable。 (这里填充DataTable的时候遇到了类型不一致的问题，某列中存在"1"和"String",SQLite貌似会识别为Int64类型，目前暂无法解决，推荐使用ExecuteReader)
        /// </summary> 
